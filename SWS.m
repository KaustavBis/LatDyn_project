% =========================================================================
% STARTUP: SWEPT SINE (CHIRP) TESTING
% Method: ISO 7401 Continuous Frequency Sweep (0.1 Hz to 3.0 Hz)
% =========================================================================
clc; close all;
disp('--- Initializing Swept Sine Trajectory and Parameters ---');
...
% -------------------------------------------------------------------------
% 2. LQR GAIN SCHEDULING (ORIGINAL AGGRESSIVE GAINS)
% -------------------------------------------------------------------------
disp('-> Calculating LQR Gains (Original Aggressive Tuning)...');
...
% -------------------------------------------------------------------------
% 3. PLANNER: OPEN-LOOP SWEPT SINE (CHIRP) & VELOCITY PROFILE
% -------------------------------------------------------------------------
disp('-> Generating Open-Loop Swept Sine Input...');
sim_params.dt = 0.001;
t_end_sim = 40.0; % Extended to 40 seconds
...
f1 = 3.0; % End frequency (Hz)
t_chirp = 20.0; % Duration of sweep
...
for i = 1:length(t_sim)
    if t_sim(i) >= t_start && t_sim(i) <= (t_start + t_chirp)
        t_active = t_sim(i) - t_start;
        % Instantaneous frequency increases linearly
        phase = 2*pi * (f0*t_active + ((f1-f0)/(2*t_chirp)) * t_active^2);
        steer_rad(i) = A_rad * sin(phase);
    end
end
...
% -------------------------------------------------------------------------
% Prevents 'X_track undefined' crashes in closed-loop visualization blocks
...
sim_params.initial_speed = 0.0; % Start from 0, allow 15s to hit 100km/h
sim_params.initial_X = 0; 
sim_params.initial_Y = 0; 
sim_params.initial_psi = 0;

disp('--- Setup Complete: Vehicle is ready for Swept Sine. ---');